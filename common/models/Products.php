...
class Products extends \common\models\base\Products {
    ...
    public function getRecordPrevious($model) {
        $hasRecord = Products::find()->count();
        if ($hasRecord) {
            if ($model->getActionUser() == 0) {
                return $hasRecord;
            } else{
                return $hasRecord + 1;
            }
    ...
    public function getIdProd($model) {
        $id = Products::find()->orderBy(['id_products' => SORT_DESC])->scalar();
        $temp = $id;
        var_dump($temp);die;
    }
}